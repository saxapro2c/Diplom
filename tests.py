...
# api.get_comments(news_id)
# api.add_comment(news_id, login, text)
...
def sync_vyatsu_news_to_db():
    try:
        ...
        for a_tag in all_a_items:
            ...
            img_tag = a_tag.find("img")
            img_url = img_tag["src"].replace("/tiny/", "/") if img_tag else None
            ...
            api.upload_news(title, description, img_url or "", link)
    except Exception as e:
        print("[ERROR] sync_vyatsu_news_to_db:", e)
...
class ProfileScreen(MDScreen):
    def on_pre_enter(self):
        ...
        self.ids.profile_label.text = (
            f"Логин: [b]{username}[/b]\n"
            f"Пароль: [b]{password}[/b]\n"
            f"Роль: [b]{role}[/b]"
        )
        # self.ids.profile_avatar.source = "profile_avatar.png"
...
class RegisterScreen(MDScreen):
    def register_user(self):
        ...
        app = MDApp.get_running_app()
        if api.register_user(username, password, role):
...
class LoginScreen(MDScreen):
    def try_login(self):
        ...
        app = MDApp.get_running_app()
        if api.check_user(username, password):
            app.current_user_login = username
            app.current_user_password = password
            app.current_user_role = api.get_user_role(username)
...
class CreateNewsScreen(MDScreen):
    ...
    def publish_news(self):
        ...
        app = MDApp.get_running_app()
        news_id = api.add_news(title, content, image_path, app.current_user_login)
...
class NewsCard(MDCard):
    ...
    def like_news(self):
        ...
        if self.news_id:
            api.add_like(self.news_id, user_login)
            self.update_like_count()

    def update_like_count(self):
        if self.news_id:
            count = api.count_likes(self.news_id)
    ...
    def update_comment_count(self):
        if self.news_id:
            comments = api.get_comments(self.news_id)
            self.ids.comment_count.text = str(len(comments))


class NewsScreen(MDScreen):
    def on_enter(self):
        self.load_news_from_api()

    def load_news_from_api(self):
        self.ids.news_feed.clear_widgets()
        news_items = api.get_all_news()

        for news in news_items:
            news_id = news.get("id")
            title = news.get("title", "")
            content = news.get("content", "")
            image_path = news.get("image_path") or "https://via.placeholder.com/300x200"
...
class TeacherScreen(MDScreen):
    ...
    def send_message(self):
        ...
        if msg:
            api.send_message(app.current_user_login, msg)
...
class EducationScreen(MDScreen):
    ...
    def like_specialty(self, specialty_id):
        app = MDApp.get_running_app()
        api.add_specialty_like(specialty_id, app.current_user_login)
        self.on_enter()

    def on_enter(self):
        self.ids.specialty_list.clear_widgets()
        specialties = api.get_all_specialties()
        ...
        for specialty in specialties:
            ...
            code = specialty["code"]
            name = specialty["name"]
            descript = specialty["descript"]
            places = specialty["places"]
            pass_score = specialty["pass_score"]
            ...
            like_count = MDLabel(
                text=str(api.count_specialty_likes(specialty_id)),
                font_style="Caption",
                size_hint_x=None,
                width="30dp"
            )
...
class CommentsPopup(Popup):
    ...
    def load_comments(self):
        ...
        for comment in comments:
            login = comment.get("user_login", "Аноним")
    ...
    def add_comment(self, *args):
        ...
        if text:
            api.add_comment(self.news_id, user_login, text)
...
class AuthApp(MDApp):
    ...
    def get_user_role(self):
        import api
        return api.get_user_role(self.current_user_login)
